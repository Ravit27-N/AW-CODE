...
@Directive({
  selector: '[cxmSmartflowPaginator]',
})
export class PaginatorDirective implements DoCheck, AfterViewInit {
  ...
  private buildPageNumbers = () => {
    // reload page when deleted the last item on the page
    if (this.matPag.pageIndex === this.matPag.getNumberOfPages()) {
      this.matPag.pageIndex--;
    }
    ...
    // First index of the button
    if (this.matPag.pageIndex < 2 && totalPages > 0) {
    ...
    const page = this.showTotalPages + 1;
    const pageDifference = totalPages - page - 1;
  ...
  private createButton(index: string, pageIndex: number): MatButton {
    ...
    switch (index) {
      case `${pageIndex}`:
        ...
        this.renderer.addClass(linkBtn, 'cxm-paginator-disabled');
