...
@Component({
  selector: 'cxm-smartflow-list-sms-template-page',
  templateUrl: './list-sms-template-page.component.html',
  styleUrls: ['./list-sms-template-page.component.scss'],
})
export class ListSmsTemplatePageComponent
  ...
  doLoadFeatureListEmailTemplate(filters: {
    ...
    this.store.dispatch(
      loadFeatureListEmailTemplate({ ...filters, templateType: 'SMS' })
    );
  }
  ...
  createTemplate() {
    this.store.dispatch(showCreateTemplatePopup({ modelType: 'SMS' }));
  }
  ...
  handleCardEvent(cardEvent: ITemplateCardEvent) {
    ...
    if (type === 'Read') {
      ...
      Promise.all([
        ...
        // this.messageService.showConfirmationMessage('feedback', '',dialog.cxmTemplate.emailTemplate.confirmDelete.title, dialog.cxmTemplate.emailTemplate.confirmDelete.body, button.cancel,'#D6D6D6', button.validate, '#D6D6D6')
        this.messageService
