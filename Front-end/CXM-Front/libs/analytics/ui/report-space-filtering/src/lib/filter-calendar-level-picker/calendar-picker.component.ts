...
import { createCustomDaterange } from '@cxm-smartflow/analytics/util';
...
@Component({
  selector: 'cxm-smartflow-calendar-picker',
...
export class CalendarPickerComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.calendarOptions && changes.calendarOptions.currentValue) {
      ...
      if (filter.option === this.customRanges.length) {
        ...
        this.setActiveDate(new Date(selectedRange?.startDate), new Date(selectedRange?.endDate));
      }
  ...
  dateOptionChanged($event: any, index: number): void {
  ...
  isSomeRange(): boolean {
    if (!this.selectedCalendar.start || !this.selectedCalendar.end) return false;
