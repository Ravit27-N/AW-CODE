...
@Component({
  selector: 'cxm-smartflow-feature-reporting-email',
  templateUrl: './feature-reporting-email.component.html',
  styleUrls: ['./feature-reporting-email.component.scss'],
})
export class FeatureReportingEmailComponent implements OnInit {
  ...
  private _setupCriteria(): void {
    ...
    const payload: any = filterHistoryManager.shouldRestoreFilterHistory('Email');
  ...
  getCriteriaOptionChange(filterOptionModel: FilterOptionModel) {
    ...
    this._store$.dispatch(setPostalCriteriaToStore({
      filteringCriteria: {
        channels: ['Digital'],
        categories: ['Email'],
