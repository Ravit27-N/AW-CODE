...
@Component({
  selector: 'app-table-report-staff',
  templateUrl: './table-report-staff.component.html',
  styleUrls: ['./table-report-staff.component.scss'],
})
export class TableReportStaffComponent implements OnInit {
  //staff
  ...
  dataSourceStaff: MatTableDataSource<CandidateAdvanceReportModel>;
  ...
  setUp() {
    ...
    this.calendarLevelChangeStaff(this.filterCalendarSelected);
  }
  ...
  getStaffReport(): void {
    ...
    const subscription = this.candidateService
      ...
      .subscribe((response) => {
        ...
        this.lengthStaff = response.total;
  ...
  //search staff
  applyFilterStaff(event: any) {
    this.dataSourceStaff.filter = event.trim().toLowerCase();
  ...
  calendarLevelChangeStaff(calendarOption: CalendarOptionModel): void {
    this.canExport = true;
    this.reportFormStaff.from = calendarOption.startDate;
    this.reportFormStaff.to = calendarOption.endDate;
    this.isInitializeStaff = false;
    this.onSubmitStaff();
  }
  exportExcel(): void {}

  async exportStaffReport(): Promise<void> {
    if (!this.canExport) {
      return;
    }
    const from = this.pipe.transform(
