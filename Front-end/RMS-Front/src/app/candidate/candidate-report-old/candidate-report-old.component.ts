...
import { AwDateFormatPipe, getAge, getAssetPrefix } from '../../shared';
...
@Component({
  selector: 'app-candidate-report-old',
  templateUrl: './candidate-report-old.component.html',
  styleUrls: ['./candidate-report-old.component.scss'],
})
export class CandidateReportOldComponent {
  displayedColumns = [
    'No',
    'Full Name',
    'Gender',
  ...
  oldFrom: Date | number | string;
  oldTo: Date | number | string;
  disButton = true;
  disSearch = true;
  ...
  getCandidateReport(): void {
    const from = this.pipe.transform(
      this.reportForm.from,
      'dd-MM-yyyy',
      'short',
    );
    const to = this.pipe.transform(this.reportForm.to, 'dd-MM-yyyy', 'short');
      ...
      .subscribe((respone) => {
        ...
        this.candidateListPDF.forEach((item) => {
          data.push({
            ...
            from: item.universities.map((x) => x?.name).join(', '),
        ...
        const ms1 =
          'From: ' +
          this.pipe.transform(this.reportForm.from, 'dd/MM/yyyy', 'short');
        const ms2 =
          'To: ' +
          this.pipe.transform(this.reportForm.to, 'dd/MM/yyyy', 'short');
        doc.text(ms1, 256, 14);
        doc.text(ms2, 275, 14);
        // // Footers
        let pageNumber = 1;
