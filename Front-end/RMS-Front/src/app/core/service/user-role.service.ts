...
import { FeatureModule, PrivilegeModel, UserRoleModel } from '../model';
...
export class ModuleService {
  ...
  get(index?: number, size?: number): Observable<IEnvelope<PrivilegeModel>> {
    let params = new HttpParams();
    params = params.set('pageSize', size.toString());
    if (index > 0) {
      params = params.set('page', index.toString());
...
export class RoleService {
  ...
  get(
    ...
    size?: number,
    filter?: string,
  ): Observable<IEnvelope<UserRoleModel>> {
    let params = new HttpParams();
    if (size > 0) {
      params = params.set('pageSize', size.toString());
    }
  ...
  delete(id: number): Observable<any> {
    return this.service.delete(`${environment.rmsContextPath}/user/role/${id}`);
  }

  validateRole(roleName: string): Observable<boolean> {
    return this.service.get(
      `${environment.rmsContextPath}/user/role/validate/${roleName}`,
    );
  }
}
