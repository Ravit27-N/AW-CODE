...
import { Ascending, Descending, Same } from '../../utils/types';
...
export const intervalGroups = [
    ...
    TSTest.group("Testing TSInterval operations", async (group) => {
        ...
        group.unary('interval.continuousWith()', async (t) => {
            t.expect0(B.continuousWith(A)).toBeTruthy() ;
            t.expect1(B.continuousWith(Y)).toBeTruthy() ;
            t.expect2(X.continuousWith(Y)).toBeTruthy() ;
            t.expect3(X.continuousWith(Z)).toBeTruthy() ;
            t.expect4(A.continuousWith(B)).toBeTruthy() ;
            t.expect5(A.continuousWith(Y)).toBeFalsy() ;
            t.expect6(Y.continuousWith(A)).toBeFalsy() ;
        }) ;
