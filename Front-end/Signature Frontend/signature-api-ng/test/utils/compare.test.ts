...
import { $arrayBufferFromBytes } from "../../utils/data";
import { TSDate } from "../../utils/tsdate";
import { TSTest } from "../../utils/tstester";
import { Ascending, Descending, Same, UINT_MAX } from "../../utils/types";
...
export const compareGroups = TSTest.group("Comparison functions", async (group) => {
    ...
    group.unary("$dateorder() function", async(t) => {
        t.expect1($dateorder(S, D)).is(Same);
        t.expect2($dateorder(S, T)).is(Same);
        t.expect3($dateorder(T, D)).is(Same);
        t.expect3($dateorder(T, undefined)).is(Descending);
        t.expect7($dateorder(A, D)).is(Descending);
        t.expect8($dateorder(T, A)).is(Ascending);
        t.expectW($dateorder(undefined, null)).toBe(Same);
        t.expectX($dateorder(undefined, undefined)).toBe(Same);
    }) ;
