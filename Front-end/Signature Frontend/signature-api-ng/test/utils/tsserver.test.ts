...
import { TSEndPointsDictionary, TSServer, TSServerOptions, TSServerRequest, TSServerStartStatus } from "../../utils/tsserver";
import { TSParametricEndPoints } from "../../utils/tsservercomp";
import { $keys, $length, $string } from "../../utils/commons";

import { TSTest } from '../../utils/tstester';
import { $absolute, $path, $readBuffer } from "../../utils/fs";
import { uint16 } from "../../utils/types";
import { $inbrowser } from "../../utils/utils";
import { Resp, RespType, TSRequest, Verb } from "../../utils/tsrequest";
...
if (!$inbrowser()) {
    serverGroups.push(TSTest.group("Testing TSServer API definitions", async (group) => {
        const localDirectory = $absolute('test/utils') ;
        const content = $readBuffer($path(localDirectory, 'main/index.html')) ;
        ...
        group.unary('Simple web page service', async (t) => {
            ...
            if (t.expect0(content).OK() && t.expect1(content!.length).gt(0)) {
                ...
                const [ret, status] = await client.request('main/index.html', Verb.Get, RespType.Buffer) ;
        ...
        group.unary('Same page in HTTP/S', async (t) => {
             ...
             */
            const cert = $readBuffer($absolute('test/utils/cert/cert.pem')) ;
            const key = $readBuffer($absolute('test/utils/cert/key.pem')) ;
