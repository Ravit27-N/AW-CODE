...
import { $arrayBufferFromBytes, $bufferFromArrayBuffer } from '../../utils/data';
import { FoundationBinaryNewLines, FoundationBynaryStrictWhiteSpaces } from '../../utils/string_tables';
import { TSData } from '../../utils/tsdata';
import { TSTest } from '../../utils/tstester';
import { Same, uint8 } from '../../utils/types';
    ...
    TSTest.group("Testing TSData", async (group) => {
        ...
        group.unary('TSData.traillingFunctions()...', async (t) => {
            ...
            if (t.expect0(WSS).OK() && t.expect1(NL1).OK()) {
                ...
                t.expectB(NL1).toBe(D) ;
                t.expectC(WSS).toBe(D) ;
                t.expectD(NL2).toBe(NL0) ;
                t.expectE(Z).toBe(D) ;
                t.expectF(Z).toBe(B) ;
