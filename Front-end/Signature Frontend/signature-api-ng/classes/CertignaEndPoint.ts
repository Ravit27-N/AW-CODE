import {$isstring, $length, $ok, stringifyPrisma} from "../utils/commons" ;
import { $ext, $isfile, $readBuffer, $uniquefile, $withoutext } from "../utils/fs";
import { HashMethods } from '../utils/crypto'
import { TSRequest, Verb, RespType, Resp, $barerauth, $basicauth } from "../utils/tsrequest";
import { TSError } from "../utils/tserrors";
import { $inspect } from "../utils/utils";
import { $left } from "../utils/strings";

import { BadRequestError, CertignaRequestError, ConflictError } from "../utils/errors";

import { APIExtensions, APIFileInfos, APIFileType, SignatureFormat, SignatureLevel, signatureLevel2Tag, SignatureLevelTag, SignatureType } from "../api/APIConstants";
...
export interface SignatureRequest {
	...
	certificateBase64Data?:Nullable<string> ;
	certificatePwd?:Nullable<string> ;
	type?:SignatureType ;
	visibleSignatureParameters?:VisibleSignatureParameters ;
}

export interface SignatureEntity {
	digestAlgorithm:HashMethods ;
...
export class Certigna extends GenericLogger {
	...
	public async certificatesAPILogin(login:string, pwd:string) : Promise<string|null> {
        let ret:Nullable<string> = undefined ;
        ...
        if (r?.status === Resp.OK) { ret = r?.response?.toString('ascii') ; } // we don't want any transformation here
	...
	public async getTOUVersion(aki:string):Promise<string|null> {
		...
		return r?.status === Resp.OK && $ok(<any>r?.response) ? (<any>r.response) as string : null ;
	}
	...
	public async generateCertificate(jwt:string, entity:CertificateRequestEntity):Promise<CertificateEntity | null> {
		...
		this.log("Generate certificate: " + stringifyPrisma(r));
	...
	public async downloadCertificate(jwt:string, SN:string):Promise<Buffer|null> {
		...
		this.log("Download certificate: " + stringifyPrisma(r));
        return r.status === Resp.OK ? r.response as Buffer : null ;
	}

	public async revoqueCertificate(jwt:string, SN:string, reason:string='API-NG end of usage revocation'):Promise<boolean> {
        const r = await this._certificateEndPoint.req(`/certificates/${SN}/revoke`, Verb.Post, RespType.String, { reason:reason }, {'Authorization':$barerauth(jwt)}, this.certificateGenerationTimeOut) ;
		this.log("Revoke certificate: " + stringifyPrisma(r));
	...
	public verifySignatureRequest(r:SignatureRequest) {
		...
		if (!$length(r.hashMethod) || !Object.values(HashMethods).includes(r.hashMethod)) {
		...
		switch (r.format) {
			...
			case SignatureFormat.XAdES:
				...
				if ($ok(r.visibleSignatureParameters)) {
		...
		if ($ok(r.certificateBase64Data)) {
			if (r.certificateBase64Data!.length < 4) { throw new ConflictError('Bad certificate data') ; }
			if (!$length(r.certificatePwd)) { throw new ConflictError('Bad certificate password') ; }
		}
		else if ($length(r.certificatePwd)) {
			throw new ConflictError('Certificate password set without any certificate data') ;
		}
	}
	...
	public async signDocument(doc:string|Buffer, request:SignatureRequest) : Promise<Buffer|null> {
		...
		switch (request.format) {
			case SignatureFormat.PAdES:
				...
				if ($ok(request.visibleSignatureParameters)) entity.visibleSignatureParameters = request.visibleSignatureParameters ;
        ...
        const r = await this._signatureEndPoint.req(
            ...
            RespType.Json,
            entity,
            {'Authorization': $basicauth(request.login, request.password)},
            this.signatureGenerationTimeOut
        ) ;
		this.log("signing the document: " + stringifyPrisma(r));
		
		// TODO: should we throw an error if outoutType is wrong ?
		if (r?.status === Resp.OK && (<any>r.response).type === outputType) {			
...
export async function verifySignature(xmlString:Nullable<string>) : Promise<boolean> {
    const xmlDoc = _parseXML(<string>xmlString) ;
    if (!$ok(xmlDoc)) { return false ; }
    return await _verifyXMLSignature(<Document>xmlDoc) ;
}

export async function verifySeal(xmlString:Nullable<string>, token:APIFileToken) {
...
function _prepareDocumentForSigning(file: string | Buffer): string {
	if (!$length(file)) {
		throw new Error('No specified file to sign') ;
	}
	let fileContent: string;
	if ($isstring(file)) {
		if (_isBase6(<string>file)) {
			fileContent = <string>file;// the file is already base64 encoded
		} else {
	...
	if (!$length(fileContent)) {
		throw new Error('Impossible to convert file to Base 64') ;
	}
	return fileContent;
}

function _isBase6(file: string): boolean {
    try {
        return !$isfile(file);
    } catch (e) {
        return true;// will be caught if file content is too large as the base64.
    }
}
...
function _signatureEntity<R extends SignatureEntity>(fileContent:string, r:SignatureRequest) : R {	
    ...
    let entity:SignatureEntity = {
        ...
        fileName:r.fileName,
        signatureLevel:signatureLevel2Tag(r.level),		
    }
    if ($length(r.certificateBase64Data) || $length(r.certificatePwd)) {
        if (!$length(r.certificateBase64Data)) { throw new Error('No specified certificate content') ;} ;
        if (!$length(r.certificatePwd)) { throw new Error('No password for the certificate') ;} ;
        entity.keyStore = {
            password:<string>r.certificatePwd,
            pkcs12Content:<string>r.certificateBase64Data,
            type:'PKCS12KeyStore'
        } ;
    }
    return entity as R ;
...
function _parseXML(data: Nullable<string>) : Document|null {
...
function _verifySealTag(d:Document, tag:string, value:Nullable<string>) : boolean
{
	if (!$length(value)) { return false ; }
	const elts = d.getElementsByTagName(tag) ;
	const val = elts[0]?.childNodes[0]?.nodeValue;
	return !(!$ok(elts) || elts.length !== 1 || val !== value);
