import { Nullable, StringArrayDictionary, StringDictionary, StringEncoding, TSDataLike } from "../utils/types";
import {
	$count,
	$defined,
	$isarray,
...
import {TSError} from "../utils/tserrors";
import {TSCharset} from "../utils/tscharset";
import {
	$absolute,
	$createDirectory,
	$ext,
...
import {Verb, RespType, Resp, TSRequest, $query, RequestHeaders} from "../utils/tsrequest";
...
import {
	...
	OTPResource,
	CGUResource,
	SessionSignDocumentNode, SessionSignDocumentsBody
...
export class NG extends TSRequest {
    ...
    public constructor(baseURL:string, authentication:APIHeaders, opts:NGOptions = {}) {
        const localtimeout = $unsigned(opts.timeOut) > 0 ? $unsigned(opts.timeOut) : NG.DEFAULT_TIMEOUT ;
        super(
            baseURL,
            {
        ...
        this.resourcesPath = $absolute($string(opts.resourcesFolder)) ;
        this.outputPath = $absolute($string(opts.outputFolder)) ;
        this.timeOut = localtimeout ;
        
        if (!!opts.createFolders) {
            [this.resourcesPath, this.outputPath].forEach(p => {
                if (!$isdirectory(p)) {
                    const creation = $createDirectory(p) ;
                    if (!!opts.debug && creation) {
                        this.log(`did create folder "${p}"`)
                    }
                    else if (!creation) {
                        throw new TSError('new NG(): Unable to create folder', {
                            options:opts,
                            folder:p,
                        }) ;
                    }
        ...
        if (!!opts.createFolders && !$isdirectory(this.resourcesPath)) { $createDirectory(this.resourcesPath) ; }
        if (!!opts.createFolders && !$isdirectory(this.outputPath))    { $createDirectory(this.outputPath) ; }

        if (!$isdirectory(this.resourcesPath) || !$isdirectory(this.outputPath)) {
            throw new TSError('new NG(): Inexistent resources folder or output folder', {
                options:opts,
                resourcePath:this.resourcesPath,
	...
	public log(message:string) {
		if (this.debug) { $logterm(`&wNG() info  : ${message}&0`) ; }
	}
	...
	public async download(downloadURL:string) : Promise<Nullable<Buffer>> {
        return await this.ngrequest<Buffer>(returnPath(downloadURL), Verb.Get, RespType.Buffer, undefined, undefined, undefined, this.downloadTimeOut) ;
	}
	...
	public async addDocument(sessionURL:string, body:CreateDocumentBody) : Promise<string|null> {
        ...
        return _ngurl(await this.ngrequest(`/${returnPath(sessionURL)}/documents`, Verb.Post, RespType.Json, [Resp.Created], body )) ;
	}
	...
	public async addActor(sessionURL:string, body:CreateActorBody) : Promise<string|null> {
        ...
        return _ngurl(await this.ngrequest(`/${returnPath(sessionURL)}/actors`, Verb.Post, RespType.Json, [Resp.Created], body )) ;
	}
	...
	public async addScenario(sessionURL:string, body:ScenarioBody) : Promise<string|null> {
        ...
        return _ngurl(await this.ngrequest(`/${returnPath(sessionURL)}/scenarios`, Verb.Post, RespType.Json, [Resp.Created], body )) ;
	}
	...
	public async createCertificate(sessionURL:string, body:CreateCertificateBody) : Promise<string|null> {
        ...
        return _ngurl(await this.ngrequest(`/${returnPath(sessionURL)}/certificates`, Verb.Post, RespType.Json, [Resp.Created], body )) ;
	}
	...
	public async documentsList(sessionURL:string, mask:number = 0) : Promise<string[]> {
		if ( !$isunsigned(mask)) { return [] ; }
        const r = await this.ngrequest($query(`/${returnPath(sessionURL)}/documents`, mask === 0 ? {} : { 'status_mask':mask })) ;
	...
	public async taggedDocuments(sessionURL:string, query:ActorDocumentsQuery|TagDocumentsQuery) : Promise<StringArrayDictionary|null> {
        ...
        return await this.ngrequest<StringArrayDictionary>($query(`/${returnPath(sessionURL)}/documents`, query)) ;
	}

	public async actorsList(sessionURL:string) : Promise<string[]> {
		const r = await this.ngrequest(`/${returnPath(sessionURL)}/actors`);
	...
	public async taggedActors(sessionURL:string, tags:string|string[]) : Promise<StringArrayDictionary|null> {
        ...
        return await this.ngrequest<StringArrayDictionary>($query(`/${returnPath(sessionURL)}/actors`, {tags:tags})) ;
	}
	...
	private async _getResource<T>(resourceURL:string) : Promise<T|null> {
		return await this.ngrequest<T>(returnPath(resourceURL));
	}
	...
	public async closeSession(sessionURL:string, body:SessionClosureBody) : Promise<ClosingSessionResource|null> {
        ...
        return await this.ngrequest<ClosingSessionResource>(`/${returnPath(sessionURL)}/close`, Verb.Put, RespType.Json, [Resp.OK, Resp.Created], body, undefined, this.signatureTimeOut) ;
	}
	...
	public async extendSession(sessionURL:string, body:SessionExtendBody) : Promise<boolean> {
		...
		return _ngbool(await this.ngrequest(`/${returnPath(sessionURL)}/extend`, Verb.Put, RespType.Json, undefined, body), "url") ;
	}

	public async getSessionManifest(sessionURL:string) : Promise<string|null> {
        // if (!$isurl(sessionURL)) { return null ; }

		return _ngurl(await this.ngrequest(`/${returnPath(sessionURL)}/manifest`, Verb.Get,RespType.Json, undefined, undefined, undefined, this.signatureTimeOut)) ;
	}
	...
	public async getCurrentDocument(documentURL:string) : Promise<string|null> {
        // if (!$isurl(documentURL)) { return null ; }
		return _ngurl(await this.ngrequest(`${returnPath(documentURL)}/current`, Verb.Get, RespType.Json, [Resp.Created])) ;
	}
	...
	public async activateScenario(scenarioURL:string, body:ManifestDataBody) : Promise<boolean> {
		...
		return _ngbool(await this.ngrequest(`/${returnPath(scenarioURL)}/activate`, Verb.Put, RespType.Json, undefined, body), "url") ;
	}
	...
	public async getCGU(caURL:string, sessionURL:string, actorURL:string) : Promise<CGUResource|null> {
        ...
        return await this.ngrequest<CGUResource>($query(`/${returnPath(caURL)}/cgu`, { session:$url2gid(sessionURL), actor:$url2lid(actorURL)})) ;
	}
	...
	public async generateOTP(sessionURL:string, body:SessionOTPBody) : Promise<OTPResource|null> {
        ...
        return await this.ngrequest<OTPResource>(`/${returnPath(sessionURL)}/generate-otp`, Verb.Put, RespType.Json, undefined, body) ;
	}

	public async checkOTP(sessionURL:string, body:SessionCheckOTPBody) : Promise<boolean> {
        if ( $length(body.otp) === 0) { return false ; }
		return _ngbool(await this.ngrequest(`/${returnPath(sessionURL)}/check-otp`, Verb.Put, RespType.Json, undefined, body), "otp") ;
	}
	...
	public async approveDocuments(sessionURL:string, body:SessionApproveDocumentsBody) : Promise<SigningResource|null> {
        ...
        return await this.ngrequest<SigningResource>(`/${returnPath(sessionURL)}/approve-documents`, Verb.Put, RespType.Json, undefined, body) ;
	}

	public async directSignDocuments(sessionURL:string, body:SessionDirectSignDocumentsBody) : Promise<SigningResource|null> {
		if ( !isValidDirectSignDocumentsBody(body)) { return null ; }
		return await this.ngrequest<SigningResource>(`/${returnPath(sessionURL)}/direct-sign-documents`, Verb.Put, RespType.Json, undefined, body, undefined, this.signatureTimeOut) ;
	}
	
	public async signDocuments(sessionURL:string, body:SessionDirectSignDocumentsBody) : Promise<SigningResource|null> {
        if ( !isValidSignDocumentsBody(body)) { return null ; }
        return await this.ngrequest<SigningResource>(`/${returnPath(sessionURL)}/sign-documents`, Verb.Put, RespType.Json, undefined, body, undefined, this.signatureTimeOut) ;
	}
...
function isValidID(id:Nullable<number|string|bigint>):boolean {
...
function isValidScenarioBody(body:ScenarioBody):boolean {
    ...
    for (let s of body.steps) {
        if (!$length(s.process) ||
            !$count(s.steps) ||
            ($ok(s.signatureType) && !Object.values(SignatureType).includes(s.signatureType!)) ||
...
function isValidSignDocumentsBody(body:SessionSignDocumentsBody):boolean {
	return $length(body.certificate) > 0 && isValidDirectSignDocumentsBody(body) ;
}

function isValidDirectSignDocumentsBody(body:SessionDirectSignDocumentsBody):boolean {
