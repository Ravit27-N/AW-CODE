...
import {
    FoundationStringEncodingsMap,
    FoundationWhiteSpacesNumberCodeSet,
    FoundationWhiteSpacesStringCodeSet
} from "./string_tables";
...
["utils", "tests", "dist"].forEach((d) => {
    if ($filename(__localDefaultPath) === d) {
        __localDefaultPath = $dir(__localDefaultPath);
    }
});

export const DEFAULT_PATH = __localDefaultPath;
export function $date2string(aDate?: Nullable<Date | string>, option?: "iso" | "normal"): string {
    if ($ok(aDate) && $isstring(aDate) && $length(<string>aDate) > 0) {
        return <string>aDate;
    }
    if (!$ok(aDate)) aDate = new Date();
    return option === "iso" ? (<Date>aDate).toISOString() : <string>aDate;
}
export function $now() { return $date2string(); }
export function $finalDateString(
...
export function $tsdateFromString(date: Nullable<string>): TSDate {
    let dt = $length(date) ? new Date(<string>date) : new Date();
    return new TSDate(dt);
}
...
export function $compareDates(
    ...
    if ($ok(A)) {
        if ($ok(B)) {
            let tA = <number>(A as Date).getTime(),
                tB = <number>(B as Date).getTime();
...
export function $timeBetweenDates(
    ...
    if ($ok(A)) {
        if ($ok(B)) {
            let tA = (A as Date).getTime(),
                tB = (B as Date).getTime();
            return (tB - tA) / 1000.0;
        }
...
export function $phone(phone: Nullable<string>, dialCode?: Nullable<string>): string {
    if ($length(phone)) {
...
export function $exit(reason: string = "", status: number = 0, name?: string) {
    if (status !== 0) {
        const processName = $length(name)
            ? name
            : `node process ${$filename(process.argv[1])}`;
        $logterm("&0&o----------------------------------------------------&0");
        if ($length(reason)) {
            $logterm(
                `&oExiting &y${processName} &owith status &y${status} &ofor reason:\n\t&p${reason}&0`
            );
        } else {
...
export function $identifier(n: number) {
    // we assume n is an unsigned
    return n > UINT16_MAX ? n.toString(16) : n.toHex4()
    
}

export function $timestampDb2client(d: Nullable<Date | string>): string {
    let timezone = new Date().getTimezoneOffset();
    let diffHours = timezone / 60;

    let date = d instanceof Date ? d : new Date(d as string);
    date.setHours(date.getHours() - diffHours);
    return date.toISOString();
}

/** ect: Central European Time */
export function $ectDateFromString(date: Nullable<Date | string>): TSDate {
    let d = date instanceof Date ? date : new Date(date as string);
    let td = d.toLocaleString("en-US", {timeZone: "Europe/Paris"});
    return new TSDate(new Date(td));
}

export const parsePrisma = <T>(json: Prisma.JsonValue | undefined): T =>{
    return JSON.parse(json as string) as T;
}

export const stringifyPrisma = <RT, T>(obj: T | undefined): RT =>{
    return JSON.stringify(obj) as RT;
}