...
export type KeysExclude<T, TT> = {
    [K in keyof T]: T[K] extends TT ? K : never
}[keyof T]

export type ModifyPropType<T, TR> = {
    [P in keyof T] : null extends T[P] ? TR | null : TR
}

export type ModifyObject<T, M, TR> = Omit<T, keyof M> & ModifyPropType<M, TR>;

export type OnlyBigint<T, TC = Prisma.BigIntFieldUpdateOperationsInput> = Pick<T, KeysExclude<T, TC | bigint | null | number | undefined>>;