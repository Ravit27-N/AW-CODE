...
import {AddMore, Participants, TitleAddMore} from '../other/common';
...
const Champ = ({
  ...
  const {setCanvasParaph, setCanvasApproved, setCanvasSignatory} = useCanvas({
    instance,
  });
  // Data from redux
  const {
    ...
    <Stack sx={{width: '100%', border: `1px solid ${grey[400]}`}}>
      ...
      {/* <Stack
        ...
        <IconButton
          onClick={() => {
            ...
            if (annoActive === 'D.AnnotationCreateFreeText') {
              instance!.UI.setToolMode('Pan');
              setAnnoactive('Pan');
            } else {
              instance!.UI.setToolMode('AnnotationCreateFreeText');
              setAnnoactive('D.AnnotationCreateFreeText');
            }
          }}
          sx={{
            borderColor:
              annoActive === 'D.AnnotationCreateFreeText'
                ? '#D6056A'
                : undefined,
            ...annoStyles,
            width: 'auto',
          ...
          <TextAnnoSVG sx={{pt: 0.5}} />
          <Typography sx={{color: '#000000'}}>
            {t(Localization('pdf-edit', 'text'))}
          </Typography>
        </IconButton>
        <IconButton
          onClick={() => {
            instance!.UI.setToolMode('AnnotationCreateStamp');
            if (annoActive === 'D.AnnotationCreateStamp') {
              instance!.UI.setToolMode('Pan');
              setAnnoactive('Pan');
            } else {
              instance!.UI.setToolMode('AnnotationCreateStamp');
              setAnnoactive('D.AnnotationCreateStamp');
            }
          }}
          sx={{
            borderColor:
              annoActive === 'D.AnnotationCreateStamp' ? '#D6056A' : undefined,
            ...annoStyles,
            width: 'auto',
          }}>
          <SixDoctsSVG sx={{pt: 0.5}} />
          <ImageAnnoSVG sx={{pt: 0.5}} />
          <Typography sx={{color: '#000000'}}>
            {t(Localization('pdf-edit', 'image'))}
          </Typography>
        </IconButton>
      </Stack>
      ...
      <Stack
        ...
        {signatories.length && annotations.length ? (
          signatories.map(({lastName: name, firstName, id}, index: number) => (
            <Stack sx={{px: '2rem'}} key={index}>
              <Participants
                avatarSx={{
                  bgcolor: bg[index],
                  color: textColor[index],
              ...
              <Box
                sx={{
                  width: '100%',
                  py: 2,
                  display: displayAnno[index][id as any] ? 'flex' : 'none',
                }}>
                <Grid
                  container
                  rowSpacing={1}
                    ...
                    <IconButton
                      ...
                      onClick={async () => {
                        if (
                          ...
                          await setCanvasSignatory(firstName!, name!, index);
                        }
                      }}
                      sx={{
                  ...
                  <Grid item xs={12} lg={6} xl={6}>
                    <IconButton
                      ...
                      onClick={async () => {
                        if (
                          annotations.find(x => x.signatoryId === id)!.dMention
                        ) {
                          instance!.UI.setToolMode('Pan');
                          store.dispatch(
                            resetAnnotationActive({signatoryId: id!}),
                          ...
                          await setCanvasApproved(firstName!, name!, index);
                        }
                      }}
                      sx={{
                        borderColor: annotations.find(
                  ...
                  {index === 0 && (
                    <Grid item xs={12} lg={6} xl={6}>
                      <IconButton
                        ...
                        onClick={async () => {
                          if (
                            annotations.find(x => x.signatoryId === id)!.dParaph
                          ) {
                            instance!.UI.setToolMode('Pan');
                            store.dispatch(
                              resetAnnotationActive({signatoryId: id!}),
                            ...
                            await setCanvasParaph(firstName!, name!, index);
                          }
                        }}
                        sx={{
                          borderColor: annotations.find(
