...
import {useCountProjectsQuery} from '@/redux/slides/project-management/project';
...
export const TabProjectDetail = ({
  ...
  const CountProjects = useCountProjectsQuery({
    filterBy: selectOption ? selectOption.selectedFilterBy : FilterBy.ENDUSER,
  });
...
const TabAllProject = (props: ITableAllProject) => {
  ...
  useEffect(() => {
    startEndDate.start && setDateStart(startEndDate.start);
  }, [startEndDate.start]);
  ...
  useEffect(() => {
    dispatch(
      setStartEndDate({
        start: dateStart,
        end: select.active === 0 ? startEndDate.start : dateStart,
      }),
    );
  }, [dateStart]);
                      ...
                      dispatch(
                        setStartEndDate({
                          start: newDate,
                            ...
                            select.active === 0 ? startEndDate.start : newDate,
                        }),
                      );
                    }}
                  />
