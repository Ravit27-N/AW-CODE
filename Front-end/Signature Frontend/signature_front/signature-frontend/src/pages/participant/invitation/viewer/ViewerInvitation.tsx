...
import {ProjectStatus, defaultColor} from '@/constant/NGContant';
...
import {validateRoleView} from '../../common/checkRole';
...
const ViewerInvitation = () => {
  ...
  React.useMemo(() => {
    if (data) {
      ...
      const validate = validateRoleView(role);
      if (validate) {
  ...
  const validateTemplateId = () => {
    navigate(`${Route.participant.viewDocument}/${id}?${queryParameters}`);
  };
                ...
                <Stack height="128px" alignItems={'center'} direction="row">
                  <Typography
                    ...
                    <Typography
                      component="p"
                      sx={{fontSize: '22px', fontWeight: 600}}>
                      {t(Localization('invitation', 'invite-to-view'))}
                    </Typography>
                    <Typography
                      component="p"
                      sx={{
          ...
          <Container
            ...
            <Stack
              ...
              {dataResponse.map((item, index) => (
                <div key={item.file64 + item.fileName}>
                  <PDFCard
                    xl={xl}
