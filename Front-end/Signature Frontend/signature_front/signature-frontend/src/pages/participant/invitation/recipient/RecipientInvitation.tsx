...
import {Participant, ProjectStatus, defaultColor} from '@/constant/NGContant';
...
import {validateRoleReceipt} from '../../common/checkRole';
...
const RecipientInvitation = () => {
  ...
  React.useMemo(() => {
    if (data) {
      ...
      const validate = validateRoleReceipt(role);
      if (validate) {
  ...
  const validateTemplateId = () => {
    navigate(`${Route.participant.viewRecipientFile}/${id}?${queryParameters}`);
  };
              ...
              <Stack
                ...
                <Stack height="128px" alignItems={'center'} direction="row">
                  <Typography
                    ...
                    {data?.actor.role === Participant.Receipt && (
                      <Typography
                        component="p"
                        sx={{fontSize: '22px', fontWeight: 600}}>
                        {t(Localization('invitation', 'invite-to-download'))}
                      </Typography>
                    )}
                    <Typography
                      component="p"
