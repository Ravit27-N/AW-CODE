...
const ViewFileToSign = () => {
  ...
  const [validatePhoneNumber, {isLoading: isPhoneNumberLoading}] =
    useValidatePhoneNumberMutation();
  ...
  const submitPhoneNumberOtp = async () => {
    if (!phoneOtp.length) {
      setInvalidPhoneNumber('OTP required.');
    } else {
      try {
        ...
        if (res.valid) {
          if (
            !data?.actor.documentVerified &&
            data!.signatureLevel === KeySignatureLevel.ADVANCE
          ) {
            navigate(
              `${Route.participant.advance.identity}/${id}?${queryParameters}`,
            );
        ...
        const {totalAttempts, valid, number} = res as {
  ...
  return (
    <InvitationLayout>
      ...
      <ConfirmPhoneNumber
        phoneNumberToggle={togglePhoneNumbr}
        handleChangeOtp={handleChangePhoneOtp}
        otp={phoneOtp}
        phoneNumber={removedPhoneNumber ?? '....'}
        submitPhoneNumberOtp={submitPhoneNumberOtp}
        invalidPhoneMessage={inValidePhoneNumber}
        validateLoading={isPhoneNumberLoading}
      />
