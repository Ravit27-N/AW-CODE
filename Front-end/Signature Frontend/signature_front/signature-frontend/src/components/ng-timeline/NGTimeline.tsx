...
export default function NGTimeline({histories}: {histories: any[]}) {
  ...
  const handlerNotCompleted = (D: HistoryType) => {
    if (
      !['COMPLETED', HistoryStatus.EXPIRED, HistoryStatus.ABANDON].includes(
        D.action,
      )
    ) {
      return username;
  ...
  return (
    <Stack alignItems={'flex-start'} width={'100%'}>
      ...
      <Timeline position="right" sx={{width: '100%'}}>
        {$isarray(tmp) &&
          tmp?.reverse().map((D: HistoryType, index: number) => {
            return (
              <Stack
                direction={'row'}
                sx={{
