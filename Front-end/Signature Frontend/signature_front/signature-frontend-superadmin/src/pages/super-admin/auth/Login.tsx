...
import {useUserLoginHistoryMutation} from '@/redux/slides/super-admin/corporateAdminSlide';
...
export default function Login() {
  ...
  const loginProcess = async (e: any) => {
    e.preventDefault();
    try {
      let roleValidate: null | UserRoleEnum = null;
      ...
      roles.forEach((item: UserRoleEnum) => {
        if (
          [UserRoleEnum.ENDUSER, UserRoleEnum.COPERATE_ADMIN].indexOf(item) > -1
        ) {
          roleValidate = item;
        }
      });
      ...
      /** close user login record in super admin
