...
import {useLazyValidateCompanyNameQuery} from '@/redux/slides/company/query/companyRTK';
...
export const companyFormDefaultValues: ICreateCompanyForm = {
  companyName: '',
  siretNumber: '',
  contactFirstName: '',
  contactLastName: '',
...
const CreateCompanyAccount = ({
  ...
  const [validateCompanyNameTrigger] = useLazyValidateCompanyNameQuery();
  const {
    control,
    formState: {errors},
  ...
  React.useEffect(() => {
    if (getValues(`fixNumber`)) {
      triggerValidation(`fixNumber`);
    }
  }, [fixNumber]);
    ...
    <Stack
      ...
      <Stack sx={{flex: 1, gap: 1}}>
        ...
        <NGInputField<ICreateCompanyForm>
          ...
          name={'companyName'}
          onAfterChange={() => {
            triggerValidation('companyName');
          }}
          placeholder={t(Localization('company-form', 'company-name'))!}
          validate={async value => {
            const isExisted = await validateCompanyNameTrigger({
              name: value,
            }).unwrap();

            return isExisted
              ? t(Localization('company-form', 'existed-name'))!
              : undefined;
          }}
        />
      </Stack>
        ...
        <Stack direction={'row'} sx={{width: '100%'}} spacing={2}>
          <Stack sx={{width: '100%'}}>
            ...
            <NGInputField<ICreateCompanyForm>
              ...
              control={control}
              eMessage={`${t(
                Localization('company-form', 'required-contact-first-name'),
              )!}`}
          ...
          <Stack sx={{width: '100%'}}>
            ...
            <NGInputField<ICreateCompanyForm>
              ...
              control={control}
              eMessage={`${t(
                Localization('company-form', 'required-contact-last-name'),
              )!}`}
