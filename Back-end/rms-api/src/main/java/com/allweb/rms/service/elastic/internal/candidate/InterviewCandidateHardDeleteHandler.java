...
@Component
class InterviewCandidateHardDeleteHandler implements Handler {
  private final InterviewElasticsearchRepository interviewElasticsearchRepository;

  InterviewCandidateHardDeleteHandler(
  ...
  public void handle(ChainContext context) {
    ...
    if (!candidateInterviewList.isEmpty()) {
      this.interviewElasticsearchRepository.deleteAll(candidateInterviewList);
    }
  }
}
